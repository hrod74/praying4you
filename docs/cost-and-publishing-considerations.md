...
- **Firebase account & project (owner decision — confirmed):** Use the **existing Firebase/Google account** (which already owns the original app and a legacy `praying4you` project) and create a **new Firebase project** for the rebuilt mobile MVP (display name **"Praying For You"**, clean mobile/MVP-oriented project ID such as `praying-for-you-mobile` chosen at setup). **Do not reuse the legacy `praying4you` project** unless a deliberate future migration/archive is decided; leave it untouched for now. Reusing it would inherit old security rules, old data, old config, and old security assumptions, and risks accidentally breaking the original app. A new project gives the mobile MVP a clean backend foundation. (See `firebase-mvp-plan.md` §18.1 #13 and `firebase-setup-checklist.md` §2–§3.)
- **Legacy data migration:** Should existing prayer requests from the legacy Firebase Realtime Database be migrated to the new Firestore data model, or should the mobile app start fresh with a clean database? Migration requires data transformation, decisions about handling anonymous records with no user ID, and sanitization of potentially unsafe legacy content (the legacy app had a stored XSS vulnerability). Starting fresh is simpler and avoids inheriting the legacy app's data quality problems. (This is consistent with the confirmed decision above to stand up a new project rather than reuse the legacy one.)
